...
export default async function loadData (context, params) {
  ...
  switch (true) {
    ...
    case (Boolean(influences) || Boolean(keywords)):
      ...
      const sets = await Promise.all(tags.map(tag => get(`${path}/${tagType}/${tag}.json`)))
      data = sets.reduce((shared, set) => intersectByProp(shared, set), sets[0]) // start with first entry, *not* an empty array
      break
    case (Boolean(theme)):
